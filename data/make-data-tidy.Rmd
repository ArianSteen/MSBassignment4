...
arblos_aldersgrp <- separate(data = df_arbl_p_alder, c('aar', 'mnd'), sep = "M", col = "tid" )
rm(df_arbl_p_alder)
```
...
```{r, echo=FALSE}
# Fern (-2019) etc
df_arbl_p_kjonn$knavn <- str_remove(df_arbl_p_kjonn$knavn, pattern = fixed(" (-2019)"))
df_arbl_p_kjonn$knavn <- str_remove(df_arbl_p_kjonn$knavn, pattern = fixed(" (1992-2019)"))
df_arbl_p_kjonn$knavn <- str_remove(df_arbl_p_kjonn$knavn, pattern = fixed(" (1994-2019)"))
```
...
df_arbl_p_kjonn <- df_arbl_p_kjonn %>%
    pivot_wider(names_from = kjonn,
    values_from = alp_k, names_prefix = c("alp_", "alp_")) %>% 
    mutate(aar = as.integer(aar),
         mnd = as.integer(mnd))
```
...
```{r, echo=FALSE}
# Fikse navn
df_arbl$knavn <- str_remove(df_arbl$knavn, pattern = fixed(" (-2019)"))
df_arbl$knavn <- str_remove(df_arbl$knavn, pattern = fixed(" (1992-2019)"))
df_arbl$knavn <- str_remove(df_arbl$knavn, pattern = fixed(" (1994-2019)"))
df_arbl <- separate(data = df_arbl, c('aar', 'mnd'), col = "tid", sep = "M")
df_arbl <- df_arbl %>%
  mutate(aar = as.integer(aar),
         mnd = as.integer(mnd))
df_arbl <- df_arbl %>%
